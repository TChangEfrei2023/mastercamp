...
public class Request {
    ...
    public ObservableList<Component> getListComponent() {
        ...
        try {
            ...
            String[] idComponentList = e.split("idComponent\":");
            String[] idElevatorList = e.split("idElevator\":");
            String[] idErrorList = e.split("idError\":");
            String[] nomList = e.split("nom\":");
            String[] statusList = e.split("status\":");

            //ID Component
            idComponentList = stringToArray(idComponentList,numPattern);

            //ID Elevator
            idElevatorList = stringToArray(idElevatorList,numPattern);

            //ID Error
            idErrorList = stringToArray(idErrorList,numPattern);

            //Nom
            nomList = stringToArray(nomList,letterPattern);


            //Status
            statusList = stringToArray(statusList,letterPattern);
            for(int i = 0; i < idComponentList.length - 1; i++){
                Component component = new Component(Integer.parseInt(idComponentList[i]),
                        Integer.parseInt(idElevatorList[i]), Integer.parseInt(idErrorList[i]),
                        nomList[i],Boolean.parseBoolean(statusList[i]));
